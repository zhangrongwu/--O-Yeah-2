...
- (void)setScrollViewSubimages:(NSArray *)array isUrl:(BOOL)isUrl{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        [tap release];
         ...
         fourPhoto *four = [[fourPhoto alloc]initWithDictionary:[self.title objectAtIndex:i]];
        
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(0, 115, 320, 25)];
        label.textAlignment = 1;
        label.font = [UIFont boldSystemFontOfSize:15];
        label.backgroundColor = [UIColor clearColor];
        label.textColor = [UIColor whiteColor];
        label.text = four.title;
